// ============================================================================
// $Id: LookAheadFunctor.java,v 1.3 2005/08/12 02:56:50 dsyrstad Exp $
 ...
 * LookAheadIterator.  This class provides the ability to prevent excessive
...
abstract public class LookAheadFunctor<T>
    extends UnaryFunctor<Iterator<? extends T>,LookAheadIterator<T>>
{
    /**
     ...
     * to wrap it again.
     */
    protected LookAheadIterator<T> wrap(Iterator<? extends T> iterator, int count) {
        if (iterator instanceof Wrapper && ((Wrapper)iterator).owner == this)
            ...
            // true in that case.
           return (LookAheadIterator<T>) iterator;
  
        return new Wrapper<T>(this, iterator, count);
    }
     ...
     * enclosing class).  
     */
    static private class Wrapper<T> extends LookAheadIterator<T> {
        private LookAheadFunctor<T> owner;
        public Wrapper(LookAheadFunctor<T> owner, Iterator<? extends T> iter, int count) {
            super(iter, count); this.owner = owner;
        }
    }
}

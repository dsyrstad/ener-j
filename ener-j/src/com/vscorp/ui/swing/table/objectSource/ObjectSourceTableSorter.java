...
public class ObjectSourceTableSorter extends TableMap {
    ...
    public int compare(int row1, int row2) {
        ...
        for (int level = 0; level < sortingColumns.size(); level++) {
            ObjectSourceSortableTableColumn column = 
                                    (ObjectSourceSortableTableColumn)sortingColumns.elementAt(level);
    ...
    public void setSortIconInHeaders(TableColumnModel columnModel, 
                                     TableColumn column, boolean ascending)
    {
        for (int i = 0; i < columnModel.getColumnCount(); i++)
        {
            ...
            JLabel label = (JLabel)renderer.getTableCellRendererComponent(null,
                               currentColumn.getHeaderValue(), false, false, -1, -1);
            
            if (currentColumn == column) //this is the column being sorted
            {
                if (ascending) 
                {
                    label.setIcon(null/* TODO Ascending icon */);
                }
                else
                {
                    label.setIcon(null/* TODO Descending icon */);
                }
            }
            else // this is not the column that is being sorted, set its icon to null
            {
                label.setIcon(null);
            }
    ...
    public void doSort(TableColumnModel model, TableColumn column)
    {
        ...
        /**
         * The original implementation uses SHIFT-CLICK to denote sorting in
         * descending order.  I modified this so that a second single click
         * will do sort in descending order.
         */
        ...
        if (sortVector.contains(column))
        {
            ...
            // After sorting in descending order the next click on this column
            // behaves just like it is the first time the column has been clicked.
            // So remove this item from the list, since it is no longer needed.
        ...
        else
        {
            ...
            // remove all elements since we clicked on a new row to sort by.
            sortVector.removeAllElements();
                        
            // add this row to the SortVector
